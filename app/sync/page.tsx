...
export default function SyncPage() {
  ...
  async function downloadConfig() {
    ...
    setMessage("Downloaded config.json. Place it beside sync_mt5.py, then start live sync.");
  }

  return <main style={{ minHeight: "100vh", padding: "1px 24px", background: "radial-gradient(circle at 80% 10%,#302b71,transparent 34%),#0c0b14" }}><div style={card}><a href="/journal" style={{ color: "#b9b7ff", textDecoration: "none", fontWeight: 700 }}>← dashboard</a><p style={{ color: "#b9b7ff", fontSize: 11, letterSpacing: 1.5, fontWeight: 800, marginTop: 35 }}>READ-ONLY LIVE SYNC</p><h1 style={{ fontSize: 38, letterSpacing: -1.8, margin: "8px 0" }}>Keep Cheetrade in sync.</h1><p style={{ color: "#b0aec0", lineHeight: 1.55 }}>The desktop helper watches the XM MT5 app every 10 seconds and sends open positions, account equity, and newly closed trades to your private journal. It contains no trading commands.</p>{user && <><button style={button} onClick={downloadConfig}>Download live sync config</button><ol style={{ color: "#c7c4ff", lineHeight: 1.7, paddingLeft: 20 }}><li>Put <code>config.json</code> beside <code>sync_mt5.py</code>.</li><li>Install <code>MetaTrader5</code> and <code>requests</code> once.</li><li>Run <code>python sync_mt5.py</code> and keep this window and XM MT5 open.</li></ol><p style={{ color: "#83e2a6", fontSize: 12, lineHeight: 1.55 }}>Your dashboard refreshes automatically. Press Ctrl+C in the sync window whenever you want to stop.</p></>}{message && <p style={{ color: "#c7c4ff", fontSize: 13, lineHeight: 1.45 }}>{message}</p>}</div></main>;
}
